...
def draw_running_time_barplot(dataset, dir: str, workload: str, comm_scale: str, passes: int, col='Row', x='Topology', y='CommsTime', hue='Topology', cut_min=False):
    # don't draw if only 1 column exists
    if len(dataset[col].unique()) <= 1:
        return

    # prepare resulting directory
    graph_dir = os.path.join(dir, f'{workload}/runtime-barplot/breakdown')
    graph_dir_cut = os.path.join(graph_dir, 'cut')
    if not os.path.exists(graph_dir):
        os.makedirs(graph_dir)
    if not os.path.exists(graph_dir_cut):
    ...
    for c in dataset[col].unique():
        data = dataset.loc[dataset[col] == c]
        ...
        title = get_plot_title(data)
        fig.suptitle(title)

        # units_count = dataset.loc[dataset['Row'] == c].iloc[0]['UnitsCount']
        # fig.suptitle(f"Row {c} ({units_count})\n{workload} (comm_scale: {comm_scale}, {passes}-pass)")
        ...
        plt.tight_layout()
        if cut_min:
            graph_path = os.path.join(graph_dir_cut, f'row{c}_{workload}_{comm_scale}commscale_{passes}pass_cut.pdf')
            plt.savefig(graph_path)
        else:
            graph_path = os.path.join(graph_dir, f'row{c}_{workload}_{comm_scale}commscale_{passes}pass.pdf')
            plt.savefig(graph_path)
        plt.clf()
        plt.close()


def draw_running_time_cost_scatter_plot(dataset, dir: str, workload: str, comm_scale: str, passes: int, x='Cost', y='CommsTime', hue='Row', style='Topology'):
    # prepare resulting directory
    graph_dir = os.path.join(dir, f'{workload}/cost-runtime')
    if not os.path.exists(graph_dir):
        os.makedirs(graph_dir)
